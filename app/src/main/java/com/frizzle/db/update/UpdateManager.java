...
public class UpdateManager {
    ...
    public void startUpdateDb(Context context){
        ...
        for(User user:userList){
            ...
            for(UpdateDb updateDb : updateDbs){
                ...
                Log.i("frizzle",user.getId()+"用户数据库升级成功");
            }
        }
    }
    ...
    private SQLiteDatabase getDb(Integer id) {
        ...
        File file = new File("data/data/com.frizzle.db/u_"+id+"_private.db");
        if(!file.exists()){
            Log.e("frizzle",file.getAbsolutePath()+"数据库不存在");
            return null;
        }
